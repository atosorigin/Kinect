...
namespace Kinect.Core.Gestures
{
    ...
    public static class GestureFactory
    {
        ...
        public static SelfTouchGesture AddSelfTouchGesture(this User user, Point3D correction,
                                                           params SkeletonJoint[] joints)
        {
            if (joints.Length < 2)
            {
        ...
        private static bool AreJointsMatching(XElement pointsToCheck, params SkeletonJoint[] givenJoints)
        {
            List<SkeletonJoint> xmlJoints = GetJoints(pointsToCheck);

            if (xmlJoints != null && xmlJoints.Count == givenJoints.Length)
            {
                foreach (SkeletonJoint joint in givenJoints)
                {
        ...
        private static List<Filter<IUserChangedEvent>> GetStandardSelfTouchFilters(SkeletonJoint[] joints)
        {
            if (joints.Length < 2)
            {
                throw new ArgumentException("At least 2 joints are expected for a SelfTouchGesture", "joints");
        ...
        private static List<SkeletonJoint> GetJoints(XElement pointsToCheck)
        {
            IEnumerable<XElement> points = pointsToCheck.Descendants();
            var joints = new List<SkeletonJoint>(points.Count());
        ...
        private static List<Filter<IUserChangedEvent>> GetFilters(XElement filters, params SkeletonJoint[] joints)
        {
